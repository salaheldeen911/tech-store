<div id="createProduct">
    <div class="row" style="border-bottom: 1px solid #c4c4c4;">
        <div class="left_side col-md-7">
            ...
            <div class="form-group">
                ...
                <div class="input_holder">
                    <input id="price" wire:model.lazy.debounce.1000ms="data.price" data-spry='integer'
                        type="number" min="0.00" max="10000.00" step="0.01"
                        class="form-control @error('data.price') is-invalid @enderror" name="price"
            ...
            <div class="form-group">
                ...
                <div class="input_holder">
                    <input id="discount" wire:model.lazy.debounce.1000ms="data.discount" type="number" min="0.00"
                        max="10000.00" step="0.01" class="form-control @error('data.discount') is-invalid @enderror"
                        name="discount" value="{{ old('data.discount') ? old('data.discount') : 0 }}">
                    @error('data.discount')
                        <span style="color:red">{{ str_replace('data.', '', $message) }}</span>
                    @enderror
                </div>
        ...
        <div class="right_side col-md-5">
            @if ($category_id == $product::$mobileCategory)
                ...
                @if ($dataDetails['smart'])
                    ...
                    <div class="form-group">
                        ...
                        <div class="input_holder">
                            <input id="ram" wire:model.lazy="dataDetails.storage" data-spry='integer'
                                type="number"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input_holder">
                            <input id="ram" wire:model.lazy="dataDetails.ram" data-spry='integer'
                                type="number" max="50" min="1"
                                class="form-control  @error('dataDetails.ram') is-invalid @enderror" name="ram"
                                value="{{ old('dataDetails.ram') }}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input_holder">
                            <input id="main_camera" wire:model.lazy="dataDetails.main_camera" data-spry='integer'
                                type="number"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input_holder">
                            <input id="front_camera" wire:model.lazy="dataDetails.front_camera" data-spry='integer'
                                type="number"
            ...
            @if ($category_id == $product::$tvCategory)
                ...
                <div class="form-group">
                    ...
                    <div class="input_holder">
                        <select id="referesh_rate" wire:model.lazy="dataDetails.referesh_rate_id"
                            name="resolution_id"
                            class="form-control @error('dataDetails.referesh_rate_id') is-invalid @enderror">
                            <option disabled selected value=null> -- Select referesh Rate -- </option>
                            @foreach ($refresh_rates as $refresh_rate)
                                <option value="{{ $refresh_rate->id }}"
                                    {{ old('dataDetails.referesh_rate_id') == $refresh_rate->id ? 'selected' : '' }}>
                                    {{ $refresh_rate->name }}Hz
                                </option>
                            @endforeach
                        </select>
                        @error('dataDetails.referesh_rate_id')
                            <span
                                style="color:red">{{ str_replace(['data details.', 'when smart is true.'], '', $message) }}</span>
                        @enderror
                    </div>
            ...
            @if ($category_id == $product::$laptopCategory)
                ...
                <div class="form-group">
                    ...
                    <div class="input_holder">
                        <input id="ram" wire:model.lazy="dataDetails.storage" data-spry='integer'
                            type="number" class="form-control  @error('dataDetails.storage') is-invalid @enderror"
                            value="{{ old('dataDetails.storage') }}">
                        @error('dataDetails.storage')
                            <span
                                style="color:red">{{ str_replace(['data details.', 'when smart is true.'], '', $message) }}</span>
                ...
                <div class="form-group">
                    <label for="ram">{{ __('Ram') }}</label>
                    <div class="input_holder">
                        <input id="ram" wire:model.lazy="dataDetails.ram" data-spry='integer' type="number"
                            max="50" min="1"
                            class="form-control  @error('dataDetails.ram') is-invalid @enderror" name="ram"
                            value="{{ old('dataDetails.ram') }}">
                        @error('dataDetails.ram')
            ...
            <div id="imagesContainer" class="container show-images-container" style="align-items: end">
                @if (count($data['images']) == 0)
                    <div class="image" style="left:65px; position:absolute; top:10px;">
                        <span role="button" wire:loading.attr="disabled" onclick="clickUpload(this)"
                            class="btn btn-dark">{{ __('Add images') }}</span>
                        <input id="imagesInput" wire:loading.attr="disabled" class="image-input"
                            wire:model.lazy="data.images" type="file" accept="image/*" min="1"
                            max="4" multiple>
                    </div>
                @endif
                @if (count($data['images']) > 0)
                    ...
                    @if (count($data['images']) !== 0)
                        @foreach ($data['images'] as $image)
                            <div class="sub-image-holder w-25 mt-1 mb-1 pl-1 pr-1" style="justify-self: end">
                                <img style="width:100%;height:100%;padding-top: 10px;" src="{{ asset($image) }}"
                                    alt="">
                            </div>
                        @endforeach
                    @endif
    ...
    <div class="row">
        <div style="margin:0 auto;">
            <button wire:loading.attr="disabled" wire:click="updateProduct" type="submit" class="btn btn-primary">
                {{ __('Update product') }}
            </button>
        </div>
    </div>
